...
CKPT_DIR  = "records/depthcycle_v11"
ARCHIVE   = "records/depthcycle_v11_archive"
BEST_CKPT = os.path.join(CKPT_DIR, "depthcycle_best.pth")
...
# 9. BLOCK
# 
class DepthCycleBlock(nn.Module):
    def __init__(self):
        super().__init__()
...
# 10. MODELL
# 
class JamOne_DepthCycle(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks     = nn.ModuleList([DepthCycleBlock() for _ in range(N_UNIQUE)])
...
def pack_int5(tensor_int8):
    ...
    """
    flat = (tensor_int8.flatten().cpu().to(torch.int32) + 16).numpy()  # 0..31
    pad  = (8 - len(flat) % 8) % 8
...
def train():
    print("-" * 70)
    print("  JamOne DEPTHCYCLE V11  -  Inline Selbstheilung")
    print("-" * 70)

    if not os.path.exists(TRAIN_PATH):
        print("[DRY-RUN] Kein Datensatz. Architektur-Test...")
        model        = JamOne_DepthCycle().to(DEVICE)
        model.count_params()
    ...
    log_f = open(f"depthcycle_v11_log_{ts}.csv", "w")
    ...
    try:
        while True:
            ...
            with open("depthcycle_live.json", "w") as jf:
                json.dump({
